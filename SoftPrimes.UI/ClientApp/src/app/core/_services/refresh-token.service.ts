...
import {AccessToken, API_BASE_URL, SwaggerClient} from './swagger/SwaggerClient.service';
...
export class RefreshTokenService {
  ...
  private refreshToken(isAuthUserLoggedIn: boolean) {
    ...
    // return this.swagger.apiAccountRefreshTokenPost(model)
    //   .pipe(
  ...
  refreshToken$() {
    this.unscheduleRefreshToken(true);
    const model = {refreshToken: this.tokenStoreService.getRawAuthToken(AuthTokenType.RefreshToken)};
    // return this.swagger.apiAccountRefreshTokenPost(model)
    //   .pipe(
    //     map(response => response || {}),
    ...
    //       this.tokenStoreService.storeLoginSession(<AccessToken>result);
    //       this.deleteRefreshTokenTimerCheckId();
    //       this.scheduleRefreshToken(true);
    //     }));
  }
  ...
  private isRefreshTokenTimerStartedInAnotherTab(): boolean {
    ...
    if (isStarted) {
      ...
      // console.log(`RefreshToken timer has already been started in another tab with tabId=${timerStat.tabId}.currentTabId=${currentTabId}.`);
    }
    return isStarted;
  }
